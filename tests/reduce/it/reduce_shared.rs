...
#[derive(Debug)]
pub struct TestCase {
    pub shape: ruda_kernel::dsl::zspace::Shape,
    pub stride: ruda_kernel::dsl::zspace::Strides,
}
...
impl TestCase {
    pub fn test_shared_sum(&self) {
        ...
        let output_handle = TestInput::builder(client.clone(), ruda_kernel::dsl::zspace::shape![1])
            .dtype(input_dtype)
        ...
        let outcome = match ExecutionOutcome::from(result) {
            ExecutionOutcome::Executed => {
                ...
                let expected = HostData {
                    ...
                    shape: ruda_kernel::dsl::zspace::shape![1],
                    strides: strides![1],
                };
