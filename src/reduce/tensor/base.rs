...
use ruda_kernel::tensor::{RudaTensor, allocation::empty_device_contiguous_dtype, initialization::zeros_client};
use ruda_core::tensor::{DType, TensorMetadata, Metadata};
use ruda_core::ir::{features::AtomicUsage, StorageType, Type};
...
fn supports_atomic_add<R: Runtime>(client: &ComputeClient<R>, dtype: DType) -> bool {
    client
        .properties()
...
pub fn sum_fallback<R: Runtime>(
    tensor: RudaTensor<R>,
    mut strategy: SumStrategy,
) -> Result<RudaTensor<R>, ReduceError> {
...
pub fn sum<Run: Runtime>(
    tensor: RudaTensor<Run>,
    strategy: SumStrategy,
) -> Result<RudaTensor<Run>, ReduceError> {
...
impl Default for SumStrategy {
    fn default() -> Self {
        ...
        #[cfg(not(feature = "tensor-reduce-autotune"))]
...
pub fn reduce<Run: Runtime>(
    mut tensor: RudaTensor<Run>,
    output_dtype: Option<DType>,
    strategy: KernelReduceStrategy,
    config: ReduceOperationConfig,
) -> Result<RudaTensor<Run>, crate::reduce::ReduceError> {
...
pub fn reduce_dim<Run: Runtime>(
    ...
    let output = init_reduce_output::<Run>(&input, dim, &dtypes, accumulator_len).ok_or(
        crate::reduce::ReduceError::InvalidAxis {
            axis: dim,
    ...
    let result = match strategy {
        KernelReduceStrategy::Unspecified => crate::reduce::reduce::<Run>(
            &client,
            input.binding(),
            output.clone().binding(),
            dim,
        ...
        KernelReduceStrategy::Specific(strategy) => crate::reduce::reduce::<Run>(
            &client,
            input.binding(),
            output.clone().binding(),
            dim,
...
pub fn init_reduce_output<Run: Runtime>(
    input: &RudaTensor<Run>,
    dim: usize,
    dtypes: &ReduceDtypes,
    accumulator_len: usize,
) -> Option<RudaTensor<Run>> {
...
pub enum KernelReduceStrategy {
    ...
    Specific(crate::reduce::launch::ReduceStrategy),
