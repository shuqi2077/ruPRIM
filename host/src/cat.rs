...
pub fn cat(tensors: Vec<HostTensor>, dim: usize) -> HostTensor {
...
fn cat_impl<E: Element + bytemuck::Pod>(tensors: Vec<HostTensor>, dim: usize) -> HostTensor {
    let dtype = tensors[0].dtype();
    ...
    if total_elements == 0 {
        ...
        return HostTensor::new(bytes, Layout::contiguous(out_shape), dtype);
    }
            ...
            .all(|t| t.layout().contiguous_offsets().is_some())
    {
        ...
        let contiguous: Vec<HostTensor> = tensors.into_iter().map(|t| t.to_contiguous()).collect();
    ...
    HostTensor::new(bytes, Layout::contiguous(out_shape), dtype)
}
